...
void SlideWindow( IplImage* img1, IplImage* img2, int cam_num, double threshold, 
  ...
  int width, height, x,y;
  IplImage* img_window = NULL;
  IplImage* dst_img = cvCreateImage( cvSize(COLS, ROWS), 8, img2->nChannels );
  int start_pos;
  Proposal one;
  double ave;
  double tmp;
  int count = COLS*ROWS*img2->nChannels;  
  int start_stride, stride;
  float* ptr;
  int img_step = ROWS*COLS*dst_img->nChannels;
  ...
  for (y=start_pos, stride = start_stride; true; y+=stride){//position
    ...
    for (x=0; true; x+=stride){
      ...
      rect.x = x; rect.y = y; rect.width = width; rect.height = height;
      ptr = NULL;
      ptr = new float[ COLS*ROWS*(img2->nChannels)*2 ];
      if (ptr == NULL){
        cout<<"error in malloc memory\n";
      }
      one.data.reset( ptr );

      cvSetImageROI( img1, rect );
      cvCopy( img1, img_window );
      cvResize( img_window, dst_img );
      ...
      for (int i=0; i<ROWS; i++){
        ...
        for (int j=0; j<COLS; j++){
          for (int c=0; c<dst_img->nChannels; c++) {
            tmp = (float)p[j*dst_img->nChannels + c]*0.00390625;
            one.data.get()[ img_step + ROWS*COLS*c + i*COLS + j ] = tmp;
            ave += tmp;
          }
        }
      }
