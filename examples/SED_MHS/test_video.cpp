...
int main(int argc, char** argv) {
  ...
  while( !list_file.eof() ) {
    ...
    // IplImage* combine_img = cvCreateImage(cvGetSize( frame ), 8, 3);
    IplImage* img1 = cvCreateImage(cvGetSize( frame ), 8, 3);
    IplImage* img2 = cvCreateImage(cvGetSize( frame ), 8, 3);
    int row, col;
    unsigned char* p1, *p2;
    int height = frame->height, width = frame->width;
    int fid_i;
    while(frame = cvQueryFrame(capture))  // 获取当前播放帧的下一帧，并且将获取到的帧加载到内存中，覆盖掉前面帧所占的内存  
    {  
      // cvCvtColor( frame, gray, CV_RGB2GRAY );
      // if (fid < 3){
      //    for(row=0; row<height; row++){
      //       p1 = (unsigned char*)( gray->imageData + gray->widthStep*row );
      //       p2 = (unsigned char*)( combine_img->imageData + combine_img->widthStep*row );
      ...
      //    continue;
      // }else{
      //   for (fid_i=0; fid_i<2; fid_i++){
      //     for(row=0; row<height; row++){
      //       p2 = (unsigned char*)( combine_img->imageData + combine_img->widthStep*row );
      //       for (col=0; col<width; col++){
      //          p2[col*3+fid_i] = p2[col*3+fid_i+1] ;
      //       }
      //      }
      //   }
      //   for(row=0; row<height; row++){
      ...
      // SlideWindow(combine_img, cam_num, threshold, 1.0, channels, result);
      if (fid < 2){
        if (fid == 0)
          cvCopy(frame, img1);
        if (fid == 1)
      ...
      result2.clear();
//cout<<"a\n";
      SlideWindow(img1, img2, cam_num, threshold, 1.0, channels, result);
