...
int main(int argc, char** argv) {
  ...
  int rows = 32, cols = 32;
  string line, save_name, video_name, detect_name;
  ...
  while( !list_file.eof() ) {
    ...
    if (detect_name[0] == 'L'){
      save_name = "feature/train/" + detect_name + ".feature"; 
      detect_name = "feature/train/" + detect_name + ".data"; 
    }else{
      save_name = "feature/test/" + detect_name + ".feature"; 
      detect_name = "feature/test/" + detect_name + ".data"; 
    }
    cout
    ...
    {  
      ...
      proposal.clear();
      for (di=0; di<num; di++) {
        w = abs( result[di].root[2] - result[di].root[0] )*1.5;
        h = abs( result[di].root[3] - result[di].root[1] )*1.5;
        rect.x = result[di].root[0] - w/3;
        rect.y = result[di].root[1] - h/12;
        rect.width = w; 
        rect.height = h; 
        if ( !(rect.x < 0 || rect.x+w>= frame->width ||
            rect.y < 0 || rect.y+h >= frame->height ||
            w == 0 || h==0) )
        {
      ...
      fwrite( (char*)&fid, sizeof(int), 1, save_file );
      fwrite( (char*)&size, sizeof(int), 1, save_file );
      ...
      for (i=size; i>0; i-=batch_size){
        ...
        for (j=0; j<deal_size; j++){
          a_result.root[0] = proposal[ i-j-1 ].x;
          a_result.root[1] = proposal[ i-j-1 ].y;
          a_result.root[2] = a_result.root[0] + proposal[ i-j-1 ].width;
          a_result.root[3] = a_result.root[1] + proposal[ i-j-1 ].height;
          a_result.confidence = proposal[ i-j-1 ].score;
          //cout<<a_result.root[0]<<" "<<a_result.root[1]<<" "<<a_result.root[2]<<" "<<a_result.root[3]<<endl;
          fwrite( &( a_result ), sizeof( Result ), 1, save_file );
...
/*
        fwrite( (char*)&fid, sizeof(int), 1, save_file );
        fwrite( (char*)&num, sizeof(int), 1, save_file );
        for (int di=0; di<num; di++) {
          fwrite( &(result[di]), sizeof( Result ), 1, save_file );
          fwrite( float_data+di*64, sizeof(float), 64, save_file );
        }
*/