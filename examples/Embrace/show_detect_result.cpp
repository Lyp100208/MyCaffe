...
bool draw_rect( IplImage* img, Result* result ,const float threshold, const int size){
  ...
  for (i=0; i<size; i++){
    ...
    if (result[i].confidence >= threshold) {
	    ...
	    sprintf(show, "%f", result[i].confidence);
	    cvPutText(
						img,
						show,
						cvPoint( result[i].root[0], result[i].root[1]-13 ),
						&font,CV_RGB(255,0,0));//在图片中输出字符 
	    flag = true;
    }
...
int main(int argc, char** argv)
{
	...
	sprintf(video_path, "%s/CAM%d/%s", video_folder.c_str(), cam_num, video_name.c_str());
    ...
    while( !feof( fptr ) )
    {
		...
		if (fid == count && num > 0){
			...
			if ( draw_rect( image, result, threshold, num ) == true ){
