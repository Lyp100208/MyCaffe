...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  // set the input blobs
  LOG(INFO) << "input size " << param.input_size();
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    LOG(INFO) << "layer_connection.bottom_size() = "<<layer_connection.bottom_size();
    ...
    LOG(INFO) << "layer_connection.top_size() = "<<layer_connection.top_size();
    ...
    LOG(INFO)<< "top_vecs_[i].size() = "<<top_vecs_[i].size();
...
// std::cin>>x;
  ...
  std::cout<<"net_output_blobs_.size() = "<<net_output_blobs_.size()<<"\n";
  //std::cin>>x;
...
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled() {
  ...
  for (int i = 0; i < layers_.size(); ++i) {
    ...
    // LOG(INFO) <<"bottom_vecs_[i].size() = "<<bottom_vecs_[i].size();
    // LOG(INFO) <<"top_vecs_[i].size() = "<<top_vecs_[i].size();
...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::MyForward(
  Dtype* data, const int len_data, Dtype* label, const int len_label) {
  layers_[0]->MyForward_gpu(bottom_vecs_[0], &top_vecs_[0], data, len_data, label, len_label);
...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::MyForward_GetFeature(
    Dtype* data, const int len_data, Dtype* label, const int len_label, const string layer_name) {
  //int x;
  layers_[0]->MyForward_gpu(bottom_vecs_[0], &top_vecs_[0], data, len_data, label, len_label);
  for (int i = 1; i < layers_.size(); ++i) {
    // LOG(ERROR) << "Forwarding " << layer_names_[i];
    // LOG(INFO) <<"bottom_vecs_[i].size() = "<<bottom_vecs_[i].size();
    // LOG(INFO) <<"top_vecs_[i].size() = "<<top_vecs_[i].size();
    // LOG(INFO) <<"layers_[i]->layer_param().name() = "<<layers_[i]->layer_param().name();
    // LOG(INFO) <<"bottom_vecs_[i] \n\tnum = "<<bottom_vecs_[i][0]->num()<<"\n"
    // <<"\tchannels = "<<bottom_vecs_[i][0]->channels()<<"\n"
    // <<"\theight = "<<bottom_vecs_[i][0]->height()<<"\n"
    // <<"\twidth = "<<bottom_vecs_[i][0]->width()<<"\n"
    // <<"\tcount = "<<bottom_vecs_[i][0]->count()<<"\n";

    if (layers_[i]->layer_param().name() == layer_name) {
      return bottom_vecs_[i];
    }
    layers_[i]->Forward(bottom_vecs_[i], &top_vecs_[i]);
    //std::cin >> x;
  }
  //LOG(ERROR)<<"layer_name is error";
  return net_output_blobs_;
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  //LOG(INFO)<<"num_source_layers = "<<num_source_layers;
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    //LOG(INFO)<<"i = "<<i<<"    "<<target_blobs.size();
    for (int j = 0; j < target_blobs.size(); ++j) {
      // LOG(INFO)<<"j = "<<j<<"\n num = "<<target_blobs[j]->num()
      // <<" chan = "<<target_blobs[j]->channels()
      // <<" heig = "<<target_blobs[j]->height()
      // <<" width = "<<target_blobs[j]->width();
