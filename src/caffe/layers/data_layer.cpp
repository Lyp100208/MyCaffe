...
void* DataLayerPrefetch(void* layer_pointer) {
  ...
  const int cropsize_w = layer->layer_param_.cropsize();
  const int cropsize_h = cropsize_w;
  const bool mirror = layer->layer_param_.mirror();

  // cout<<"cropsize_w = "<<cropsize_w<<"\tcropsize_h = "
  //         <<cropsize_h<<"\tmirror = "<<mirror
  //         <<"\tcount = "<<layer->prefetch_data_->count()
  //         <<"\twidth = "<<layer->prefetch_data_->width()
  //         <<"\theight = "<<layer->prefetch_data_->height()
  //         <<"\tnum = "<<layer->prefetch_data_->num()<<endl;

  if (mirror && cropsize == 0) {
    LOG(INFO)<<"fatal"<<mirror<<" "<<cropsize;
    ...
    if (cropsize) {
      ...
      if (mirror && rand() % 2) {
        ...
        for (int c = 0; c < channels; ++c) {
          for (int h = 0; h < cropsize_h; ++h) {
            for (int w = 0; w < cropsize_w; ++w) {
              top_data[((itemid * channels + c) * cropsize_h + h) * cropsize_w
                       + cropsize_w - 1 - w] =
                  (static_cast<Dtype>(
        ...
        for (int c = 0; c < channels; ++c) {
          for (int h = 0; h < cropsize_h; ++h) {
            for (int w = 0; w < cropsize_w; ++w) {
              top_data[((itemid * channels + c) * cropsize_h + h) * cropsize_w + w]
                  = (static_cast<Dtype>(
                      (uint8_t)data[(c * height + h + h_off) * width
                                    + w + w_off])
                     - mean[(c * height + h + h_off) * width + w + w_off])
      ...
      if (data.size()) {
        //LOG(INFO)<<"data size = "<<data.size()<<" mean = "<<mean[0]<<"  "<<mean[783];
...
void DataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int cropsize_w = cropsize;
  const int cropsize_h = cropsize_w;
  if (cropsize > 0) {
    (*top)[0]->Reshape(
        this->layer_param_.batchsize(), datum.channels(), cropsize_h, cropsize_w);
  ...
  LOG(INFO)<<"prefetch_data_->mutable_cpu_data();";
