...
void AccuracyLayer<Dtype>::SetUp(
  ...
  // LOG(INFO)<<"loss layer batchsize = "<<batchsize;
  // LOG(INFO)<<"bottom num = "<<bottom[0]->num();
  // LOG(INFO)<<"bottom2 num = "<<bottom[1]->num();
  (*top)[0]->Reshape(bottom[0]->num()+1, 3, 1, 1);
  //(*top)[0]->Reshape(200+1, 3, 1, 1);
}
...
void AccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  //LOG(INFO)<<"num = "<<(*top)[0]->num()<<"count = "<<(*top)[0]->count();
  ...
  for (int i = 0; i < num; ++i) {
    ...
    //std::cout<<"\tmax_id = "<<max_id<<" : "<<static_cast<int>(bottom_label[i])<<" maxval = "<<maxval<<"\n";
    correct = 0;
    if (max_id == static_cast<int>(bottom_label[i])) {
      ++accuracy;
      correct = 1.0;
      
      if (static_cast<int>(bottom_label[i]) == 1)
        pos_correct++;
    }
    if (static_cast<int>(bottom_label[i]) == 1)
        pos_count++;
    ...
    (*top)[0]->mutable_cpu_data()[ (i+1)*3+0 ] = correct;
    (*top)[0]->mutable_cpu_data()[ (i+1)*3+1 ] = log_tmp;
    (*top)[0]->mutable_cpu_data()[ (i+1)*3+2 ] = maxval;


    logprob -= log_tmp;
  }
  //LOG(INFO) <<"num = "<<pos_count<< " pos_correct: " << pos_correct;
  (*top)[0]->mutable_cpu_data()[0] = accuracy / num;
  //(*top)[0]->mutable_cpu_data()[1] = logprob / num;
  (*top)[0]->mutable_cpu_data()[1] = (float)pos_correct / pos_count;
}
