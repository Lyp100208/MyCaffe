...
void ConvolutionLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO)<<"\tKSIZE_ = "<<this->layer_param_.kernelsize()<<std::endl
  <<"\tSTRIDE_ = "<<this->layer_param_.stride()<<std::endl
  <<"\tGROUP_ = "<<this->layer_param_.group()<<std::endl
  <<"\tPAD_ = "<<this->layer_param_.pad()<<std::endl
  <<"\tNUM_ = "<<bottom[0]->num()<<std::endl
  <<"\tCHANNELS_ = "<<bottom[0]->channels()<<std::endl
  <<"\tHEIGHT_ = "<<bottom[0]->height()<<std::endl
  <<"\tWIDTH_ = "<<bottom[0]->width()<<std::endl
  <<"\tNUM_OUTPUT_ = "<<this->layer_param_.num_output();
  ...
  //LOG(INFO)<<"height_out = "<<height_out<<"\twidth_out = "<<width_out;
  col_buffer_.Reshape(1, CHANNELS_ * KSIZE_ * KSIZE_, height_out, width_out);

  //col_buffer_.Reshape(1, 1, HEIGHT_, WIDTH_);
  // Set the parameters
